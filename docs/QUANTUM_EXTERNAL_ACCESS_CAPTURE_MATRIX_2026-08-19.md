...
**Purpose:** Manage all real-QPU access routes under one provider-neutral capture plan without treating outreach, account setup, proposals, or program acceptance as hardware evidence.
...
- No IBM Quantum provisioning/account notice was found in the connected mailbox, so access is not assumed.
...
**P4 — AQT:** pursue if collaboration response is positive; strong evidence depth but less predictable scheduling.  
**P5 — Braket:** activate if AWS access is established; useful for rapid provider diversity and a managed evidence path.  
