...
Claims-controlled ingest. The Navy SBIR topic reproductions below are used as secondary requirement-discovery sources; operative submission requirements must be checked against DSIP/DoW before proposal freeze.
...
These secondary topic reproductions support requirement discovery and traceability. Direct DSIP/DoW solicitation review remains the authoritative proposal-freeze gate.
...
- Government Phase-II target captured from the cited topic reproduction: >=95% conversion accuracy and <=1% error after correction; this is a future validation target, not current WS performance.
