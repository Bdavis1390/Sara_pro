"""WS-QPOS-FAMILY bounded preservation model for major proof-of-stake families.

The adapters in this module do not claim that the listed production networks
currently support post-quantum validator credentials. They encode a conservative
migration obligation: replacing cryptographic authorization must not silently
change stable validator/economic identity, consensus weight, ownership,
delegation, penalty history, rewards, or the validator's membership window.
...
PROFILES: dict[str, PosFamilyProfile] = {
    ...
    "SOLANA": _profile(
        ...
        "vote/stake account indirection exists; consensus-key transition still protocol-specific",
    ...
    "CARDANO": _profile(
        ...
        "no PQ consensus migration confirmed in reviewed official sources",
    ...
    "COMETBFT": _profile(
        ...
        "current supported key types reviewed are classical; no PQ validator key type confirmed",
...
def vote_weight_once(states: Sequence[FamilyValidatorState], identities: Iterable[str]) -> int:
    ...
    return sum(int(state.protected_map()["consensus_weight"]) for identity, state in unique.items() if identity in seen)
...
def run_family_preservation_proof() -> FamilyProofReport:
    ...
    return FamilyProofReport(
        ...
        excluded_claims=(
            ...
            "No proof of any listed chain's production consensus implementation, liveness, finality, fork choice, VRF, aggregation, or networking under PQ load.",
            ...
            "No claim that this finite adapter set literally enumerates every proof-of-stake network in existence.",
        ),
    )
