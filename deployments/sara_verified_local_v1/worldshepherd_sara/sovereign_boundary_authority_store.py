...
class PrimeEffectAuthorizationStore:
    ...
    Claim transitions are persisted under DurableStore.transact_registry(), so
    concurrent callers cannot both derive a VERIFIED -> CLAIMED transition from
    the same registry snapshot. The claim is persisted *before* the caller is
    handed a claim-bound envelope.

    The external physical side effect is intentionally not part of the file
    transaction. If execution outcome becomes uncertain after claim, callers
    must move the claim to INDETERMINATE rather than retry it automatically.
    """

    def __init__(self, store: DurableStore) -> None:
        self.store = store

    def register_verified(self, verified: VerifiedPrimeEffectAuthorization) -> dict[str, Any]:
