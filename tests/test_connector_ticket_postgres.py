...
def test_claim_sql_is_single_statement_compare_and_set():
...
def test_register_sql_fails_closed_on_duplicate_by_rowcount():
...
def test_status_query_reads_only_claim_state():
    normalized = " ".join(STATUS_SQL.split()).lower()
    assert "ticket_sha256, expires_at, consumed_at" in normalized
