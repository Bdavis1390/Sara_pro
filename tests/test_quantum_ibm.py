...
def _hardware_result(**overrides):
    payload = dict(
        ...
        instance="crn:v1:bluemix:public:quantum-computing:us-east:a/test:instance::",
        instance_plan="open",
...
def test_open_plan_ibm_result_converts_to_structurally_valid_sara_gate_record():
    ...
    assert evidence.metadata["plan_name"] == "open"
    assert evidence.metadata["plan_verification"] == "service_resolved_before_submission"
    assert evidence.metadata["instance"].startswith("crn:")
...
def test_ibm_campaign_record_refuses_unverified_instance_plan():
    with pytest.raises(ValueError, match="instance plan"):
        build_sara_qpu_external_evidence(
            _hardware_result(instance_plan=None),
            plan_name="open",
            cost_usd=0.0,
        )


def test_ibm_campaign_record_refuses_plan_mismatch():
    with pytest.raises(ValueError, match="does not match expected plan"):
        build_sara_qpu_external_evidence(
            _hardware_result(instance_plan="premium"),
            plan_name="open",
            cost_usd=0.0,
        )
