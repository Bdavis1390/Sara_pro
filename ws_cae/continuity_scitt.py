...
def _manifest_content_id(manifest: dict[str, Any]) -> str:
...
def build_statement(envelope: dict[str, Any], issuer: str, observed_at: str | None = None) -> dict[str, Any]:
    ...
    if not valid_content_id(content_id):
        raise ValueError("continuity manifest requires a canonical lowercase sha256 content_id")
    manifest = envelope.get("manifest")
    if not isinstance(manifest, dict):
        raise ValueError("continuity manifest payload is missing")
    if _manifest_content_id(manifest) != content_id:
        raise ValueError("continuity manifest content_id does not match manifest payload")
    issuer = issuer.strip()
    if not issuer:
        raise ValueError("issuer must be non-empty")
    timestamp = observed_at or _utc_now()
    if not valid_datetime(timestamp):
        raise ValueError("observed_at must be an ISO-8601 datetime with an explicit timezone")
