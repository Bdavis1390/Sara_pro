...
printf '[selection] unit/integration tests\n'
(
  cd "$DEPLOY"
  pytest -q | tee "$OUT/tests/pytest.txt"
)
...
export REPO_ROOT OUT SOURCE_SHA EXECUTED_UTC
python - <<'PY'
...
checks = {
    'pytest_executed': (out/'tests/pytest.txt').is_file(),
...
result = 'BASELINE_PASS_WITH_UNRESOLVED_GATES' if all(v for k,v in checks.items() if k != 'open_source_license_present') else 'BASELINE_FAIL'
record = {
    'schema': 'WS-PREFERRED-CHOICE-SELECTION-BASELINE-V1',
    'result': result,
    ...
    'claims_boundary': 'Passing this baseline does not establish preferred vendor status, contract selection, open-source licensing, operational-scale performance, CMMC/NIST/SPRS status, clearance, ATO, or authorization to process controlled information.',
    'artifacts': artifacts,
}
