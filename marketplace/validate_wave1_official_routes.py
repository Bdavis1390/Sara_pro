...
EXPECTED_IDS = ["WS-MKT-025","WS-MKT-001","WS-MKT-030","WS-MKT-031","WS-MKT-002","WS-MKT-019","WS-MKT-026","WS-MKT-032","WS-MKT-035","WS-MKT-036"]
...
def main():
    ...
    a=ap.parse_args()
    r=json.loads(Path(a.registry).read_text())
    g=json.loads(Path(a.gates).read_text())
    errors=[]
    if r.get('schema')!='WS-MARKETPLACE-WAVE1-OFFICIAL-ROUTE-REGISTRY-V1': errors.append('schema')
    ...
    ts=r.get('targets') or []
    ids=[x.get('id') for x in ts]
    if ids!=EXPECTED_IDS: errors.append('ids')
    gate_by_id={x.get('id'):x for x in (g.get('targets') or [])}
    for t in ts:
    ...
    out={
      'schema':'WS-MARKETPLACE-WAVE1-OFFICIAL-ROUTE-REPORT-V1',
      'result':'PASS' if not errors else 'FAIL',
      'route_target_count':len(ts),
      'verified_official_route_count':sum(1 for t in ts if t.get('verified_official_route') is True),
      'route_used_count':sum(1 for t in ts if t.get('route_used') is True),
      'external_gate_closures':0,
      'contact_gate_effect':r.get('contact_gate_effect'),
      'errors':errors,
      'claims_boundary':'PASS verifies official public route records and non-use only. It does not authorize outreach, establish supplier/partner eligibility, target interest, remediation probability or close any external gate.'
    }
